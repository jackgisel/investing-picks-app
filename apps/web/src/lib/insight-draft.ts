...
## Output
- \`bodyMd\` is GitHub-flavoured markdown containing ONLY the six sections: \`## Heading\` plus paragraphs, bullet lists, **bold**, and links. No front matter, no title (that is its own field), no closing disclaimer (the site adds one).
- \`lede\` is a single opening sentence or two, rendered above the body in larger type. It is not part of \`bodyMd\`.
- \`tldr\` is exactly five short bullets — the note in brief.
